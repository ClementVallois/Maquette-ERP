...
// Float-producing arithmetic on money — the mechanical form of `BUILD-RULES.md` § Money.
// ADR-0035 decides what it bans and what replaces each ban. Split in two because the two halves have different scopes: the calls are
...
export default tseslint.config(
  {
    ignores: [
      ...
      '**/.next/**',
      '**/*.d.ts',
      // Stryker's own working directory and HTML report (ADR-0027): instrumented
      ...
      // Generated by the TanStack Router Vite plugin on every `dev`/`build` (ADR-0062):
...

  {
    ...
    // which lives in `apps/api/` (ADR-0020). Its errors are typed by
