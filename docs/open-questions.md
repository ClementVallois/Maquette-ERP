...
| 04/09/2026 | **`docs/HANDOFF-qa-round-3.md` is an untracked 21 KB session-handoff prompt sitting in `docs/`**, absent from the README inventory that exists « pour qu'un fichier ne s'y ajoute pas en silence ». It carries an absolute local path, the instruction that a coding agent was told to skip tests and ADRs, and a reference to a `docs/qa-round-3-notes.md` that does not exist.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Committed, it publishes working method as if it were documentation, on the one artefact class CLAUDE.md holds to the highest standard. Left where it is, it is one `git add .` from doing so.                                                                                                                                                                                                                                                                                                                                                                             | **Open — Clement's call, by 30/09/2026 (Phase 10, task 10.1).** The recommendation of both review subagents is the same: it does not belong in git, and belongs in the private notes archive (ADR-0014's precedent) rather than in `docs/`. Left in place because deleting someone's working notes is not an agent's decision to take.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| 07/09/2026 | **Two commits on this branch carry an ADR each and hundreds of lines across unrelated concerns, under a one-line subject and no body.** Raised by the same `rules-auditor` pass. `94957f4` (20 files, +811/−194) bundles at least seven steps — the persona pending-state rewrite, a change to the shared kit `Button` that **removed the press affordance of every button in the application**, the mobile CRA day-card rework, the assignment-form redesign, a timeline fix, the invoice-detail rework, plus **ADR-0099** and a 47-line `docs/todo.md` edit — and its subject names two of them. `f1dc2bb` (7 files, +528/−27) carries **ADR-0100**, a 267-line spec and 223 lines of implementation. The rationale for the global `Button` change exists nowhere in git. | BUILD-RULES: "One commit = one step defensible out loud" — the history is part of the deliverable, and this repository hands the history to a reader who has no brief. The contrast is the evidence: the other 19 commits in the range carry careful bodies, and the two outliers are the two shipping the ADRs. | **Open — Clement's call, by 30/09/2026 (Phase 10, task 10.1), and the only finding in this round that cannot be fixed forward.** The branch is pushed to `origin` and the repository has been public since 03/09/2026, so adding the bodies means rewriting published history and force-pushing — a decision an agent does not take. Three ways out: reword the two commits and force-push; split them and force-push; or leave the history and accept that two commits do not meet the rule, recorded here rather than silently. The window for the first two closed on 07/09/2026, when the branch was merged: a merge fixes the history in place, so what Phase 10 decides is whether the third option — leaving it recorded — is the answer or whether the range is rewritten wholesale. |
| 07/09/2026 | **`docs/todo.md` is a third French file the 05/09/2026 row above does not name.** That row books `docs/audit-produit-ui-ux.md` and `docs/plan-densification.md` against BUILD-RULES' "everything in English except `README.md`"; `docs/todo.md` belongs on the same list and was missed. Noted rather than folded in silently, because this branch's own 42-line addition to it is **in English** and replaced a French line with English — the file is already drifting toward the rule, one section at a time, with nothing recording that as a plan. | Small on its own. It matters because the 05/09 row's three options (translate, widen the exception by ADR, or narrow the rule to what it protects) are priced per file, and a list that is short by one prices the decision wrong. | **Open — Clement's call, by 30/09/2026 (Phase 10, task 10.1), the same date as the 05/09/2026 row it completes.** No separate decision: whatever that row decides for the other two files decides this one, and this exists only so the inventory it is priced from is complete. |
| 07/09/2026 | **ADR-0103 fixed the CRA-write races that corrupt or overwrite state, and left one gap open by design: `recordMonth`'s draft/draft "last write wins" has no staleness detection.** Two overlapping edits of the same still-draft month no longer merge into a corrupted row (the lock ADR-0103 adds fixes that), but the second save now silently and completely discards the first save's edits if the two callers' `command.entries` were computed from different snapshots of the month — there is no `expectedRevision`/`If-Match` on `recordMonth`'s wire contract to refuse that, and no client concept of "the server moved since I last read". | A consultant editing the same month in two tabs, or editing while a slow save from an earlier action is still in flight, can lose an entry with no error and no warning — the request that loses returns `200`, indistinguishable on the wire from an intentional replace. | **Open — decided by `docs/clean-up-audit.consolidated.local.md` package 11 ("Complete mutation invalidation and protect unsaved CRA drafts"), no calendar date fixed in that plan yet.** Package 11 already owns the client-side half of this exact question (preserving dirty state, presenting a conflict when newer server data arrives); adding a server-side `expectedRevision` guard without package 11's client story to consume its refusal would be a guard nothing reads the answer of, per ADR-0103's own "Reconsideration threshold". |
| 07/09/2026 | **`PgReferenceReader.seller()`'s unscoped `SELECT * FROM public.legal_entities ORDER BY id LIMIT 1` (correct for the one-seller production database) makes every integration test file that drafts an invoice depend on winning an alphabetical race against every other file's committed `legal_entities` row.** Found while closing package 06: `cra-concurrency.int.test.ts` and `pg-numbering-counter.int.test.ts` each leave a permanently committed row behind (by design, to survive their own tests' rollbacks), and a `pnpm run db:reset` changed which row sorts first — silently breaking two other files' seller-identity assertions (`api.int.test.ts`, `apps/api/src/web/pre-facturier.int.test.ts`) that had never actually been testing what they claimed to, only what happened to win that day. Fixed narrowly for those two files (`000-`-prefixed ids that sort first, matching the `aaa-` convention `cra-concurrency.int.test.ts` already established) rather than broadly, because a general fix touches `PgReferenceReader.seller()` itself or every one of the nine files that seed a `legal_entities` row, and package 06's mandate is invoice-history freezing, not test-suite isolation. | A new integration test file that seeds its own seller and asserts on it can silently pass today and start failing the next time DB state or file execution order shifts which row sorts first — the exact failure mode this row documents, not a hypothetical one. | **Open — no phase named yet.** Candidates: scope `seller()`'s query in test-only setups, give the harness a single shared seller fixture every file reads instead of each seeding its own, or accept the `000-`/`aaa-` sorting convention as documented practice and add it to the harness's own guidance. Package 26 ("Align the test portfolio with the actual risks") is the most likely home; not assigned there yet because that package has not been scoped. |
| 07/09/2026 | **Package 11's `canEdit` lock (`cra-grid-screen.tsx`, `affd3a0`) — every editing path disabled while `saveMonth.isPending`, closing the window where an edit made during an in-flight save gets silently discarded once that save's own refetch lands — has no automated regression test.** Verified by reading every one of the seven `data.editable` usages in the file and confirming the edit-affecting ones (not the two that stay gated on `data.editable` alone: the informational empty-month hint, and the Save/Submit button row itself, which uses `saveMonth.isPending` through its own `pending` prop instead) now read `canEdit`. `decideGridResync` (the sibling fix in the same commit) has its own direct unit test, because it is a pure function; `canEdit` is a boolean wired into seven JSX conditionals in a component this repository has no way to render in a test — no `@testing-library/react`, no Playwright coverage of `cra-grid-screen.tsx`'s save-in-flight behavior specifically. | A future edit to this file that reintroduces `data.editable` at one of those seven sites in place of `canEdit` — a merge conflict resolved the wrong way, a copy-pasted block from before this fix — reopens the exact bug this commit closed, and nothing would fail. | **Open — the next package that adds `@testing-library/react` or extends Playwright coverage to `/cra/:period` (whichever lands first) is the deciding phase**, per the same reasoning `docs/adr/0112-persona-changes-broadcast-across-tabs-through-storage.md`'s own Reconsideration threshold gives for not building that infrastructure speculatively: add a regression test for this lock at that point, against the concrete harness, rather than standing up React rendering now for one boolean. |
---
...
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 07/09/2026 | **Package 09 of the cleanup audit added the same narrow-cast helper (`craRowStatus`/`invoiceRowStatus`/`craActivityStatus`/`invoiceActivityStatus`/`craLineQuarterDays`) independently in four route files** (`dashboard.ts`, `pre-facturier.ts`, `invoices.ts`, `cra.ts`) rather than once, shared. Each narrowed the same real gap — a repository interface (`CraListItem.status`, `InvoiceListItem.status`, `InvoiceYearStatusCount.status`, `CraLine.quarterDays`) declared wider (`string`/`number`) than the `CHECK` constraint or business rule the column actually carries.                                                                                                                                                                                                                                                                                                                                                                                                                                                    | **Settled 07/09/2026, in package 14, exactly as this row said it would be.** The discriminator the row named was run for real: `CraListItem.status` changed from `string` to `CraStatus` and `pnpm run -s typecheck` cascaded to exactly one place, `pg-cra-repository.ts`'s own row mapper — confirming the repository interface, not the domain, was the thing lying. Same result for `InvoiceListItem.status` and `InvoiceYearStatusCount.status` against `InvoiceStatus` (`pg-invoice-repository.ts`, two mappers). All three interfaces now carry their real union; the cast moved to the one PostgreSQL row mapper each actually needs it (`pg` hands every column back as `string` regardless of a `CHECK` constraint, which is the one place a cast is warranted). `craRowStatus`, both copies of `invoiceRowStatus`, `craActivityStatus` and `invoiceActivityStatus` were then all genuinely dead — deleted from `cra.ts`, `dashboard.ts`, `invoices.ts` and `pre-facturier.ts`, along with one more the row's own audit missed: `RecordMonthOutcome.status` (`apps/api/src/chain/record-month.ts`) widened `Cra.status` (already `CraStatus`) back to `string` for no reason at all, no repository involved — narrowed too, and `composition/pre-facturier.ts`'s own now-unnecessary cast on the same field went with it (caught by `no-unnecessary-type-assertion`, not by hand). `craLineQuarterDays` (`cra.ts`) is the one helper that stays, and for a different reason than the other four: `CraLine.quarterDays`'s type, `QuarterDays`, is deliberately `number` because the same type carries an aggregate month total — unlike `CraStatus`/`InvoiceStatus`, there is no narrower union sitting one property away waiting to be exposed; splitting a genuine `1 \| 2 \| 3 \| 4` cell type off the aggregate type would be a real domain-modelling decision, out of this package's scope. Full test suite (680 unit, 294 integration) green throughout, no behavior changed. |
| 04/09/2026 | **`apps/web/src/assets/security-alert.webp` is untracked, unreferenced and un-ignored.** It appeared in the working tree during the QA round 4 session, from outside that branch's work. Nothing in `.ts`, `.tsx`, `.md` or `.json` names it, so Vite emits nothing for it and it ships nowhere.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | **Settled 04/09/2026 — [ADR-0096](adr/0096-the-company-news-carousel-gets-fixed-arrows-and-a-css-timer.md).** The second of the row's own two clean answers: wired to the screen it was meant for, the company-news module's security item (item 1a, QA round 5), `import`ed rather than left as a bare path, and committed. `news-securite.svg`, the illustration it replaces there, is now itself unreferenced — left in place, not deleted, and not a new open row: an unused asset that is still tracked and ignored by nothing carries none of this row's risk (a stray `git add .` cannot pick up what is already committed).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 04/09/2026 | **The pré-facturier's own row-level "Valider" button opens a confirmation dialog with no flagged-days warning, while the CRA detail view's does.** Item 28 added `flaggedDaysCount` to `ValidateConfirmDialog`; `manager-cra-grid-screen.tsx` has `data.flags` already and passes it, `pre-facturier-screen.tsx` has no such data. `PreFacturierCraRow` (`apps/api/src/composition/pre-facturier.ts`) never runs the submission checks per row — it is a lightweight paginated summary by design.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | **Settled 04/09/2026 — [ADR-0095](adr/0095-not-computed-and-zero-are-different-answers.md).** None of the three options as stated: the count is still not computed per row (the cost argument held), and the detail dialog keeps its warning (removing it would have made the two paths agree by degrading the right one — that alternative is closed, not left standing). Instead `flaggedDaysCount` became `number                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | null` and **required**, so « not computed » renders a muted advisory where « computed, and it is zero » renders nothing, and a future caller can no longer inherit silence by saying nothing. |
| 04/09/2026 | **A consultant can be attached to a manager in another office, and then no one can act on their Cra through the UI.** `scripts/lib/seed-data.ts` carries two such rows — Gabrielle (Bordeaux) reports to Bruno (Paris), François (Rennes) to Emma (Lyon) — written before offices scoped anything. `validate-cra.ts` asks the hierarchy who may accept a Cra, and answers Bruno; every read Bruno makes is bounded by `actor.officeId`, and so never shows him Gabrielle's Cra. ADR-0090 decided the _read_ side (the team panel stops at the office) and deliberately did not decide this.                                                                                                                                                                                                                                                                                                                                                                                                                                            | **Settled 04/09/2026 — [ADR-0094](adr/0094-a-consultants-manager-is-in-the-consultants-office.md).** The first of the three options the row named: the seed stops crossing offices. Gabrielle reattaches to Karim, Bordeaux's own manager; Rennes — which had one consultant and no manager at all, which is why this was never a one-line data edit — gains its own, non-selectable, modelled on Karim; François reattaches to him. `validate-cra.ts` is untouched, so ADR-0034's dated attachment survives, and ADR-0042's office scope is not widened. This fires ADR-0090's second reconsideration threshold, which anticipated exactly this: the distinction is now proved only by `org-chart.int.test.ts`'s own fixture, and ADR-0090's closing illustration (« the panel shows him nineteen ») became historical the day it was resolved — corrected in ADR-0094 rather than in ADR-0090, per ADR-0045.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 04/09/2026 | **The README's front matter describes an application that no longer exists, and its quickstart 404s.** Four statements, all of them true before the front-end merge of 28/08/2026 and none of them since: « Où en est cette maquette » says the interactive React SPA is unwritten (it ships, and `apps/api/src/web/routes.ts` now registers two server-rendered pages, not seven); « Démarrer » gives `pnpm run setup` then `pnpm run api` and sends the reader to `http://127.0.0.1:3000/`, but never says to build the SPA, and `apps/api/src/web/spa.ts` serves the ordinary 404 when `apps/web/dist/index.html` is absent — which it is on every fresh clone, `dist/` being gitignored; « Cinq paquets » lists five where the README's own `pnpm run boundaries` prints six; and `docs/frontend-plan.md` and `docs/direction-visuelle.md` are advertised « en français comme ce README » while both are English. Found by the `cold-reader` pass of 01/09/2026, whose walk stalled on the 404 before it could read anything else. | **Settled by Phase 9's documentation pass, 04/09/2026** — the pass the row itself named, run because the repository link is going out. Every one of the four statements was re-verified against the code and corrected in `README.md`: the SPA is described as shipped and the two server-rendered documents named as the only two (`/facture/:id`, `/releve/:id`); the front-end build step is stated as non-optional in « Démarrer » **and** in the « À l'écran » walkthrough, with the 404 it causes named; « Cinq paquets » became six, which is what `pnpm run boundaries` prints, and `apps/web` was added to the diagram; the two plan documents are now described as English, which they are. Three further corrections the row did not list: the phase status (0 to 8 done, 9 in progress) — it still said « Phases 1 à 6 »; the test counts, recounted the same day (611 and 225, against 511 and 167 dated 22/08/2026); and the walkthrough's destination, which said the navigation leads to the pré-facturier where choosing a persona lands on the dashboard. A full inventory of `docs/` replaced the partial one, so a file can no longer be added there in silence.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
## QA round 4 checkpoint — `fix/qa-round-4-doc-and-scope`, 04/09/2026

Not a QA round in the sense the other three were: nothing here came from a fresh read of the running
application. It closes three rows that were already written down — the README front matter
(01/09/2026) and the two dated 04/09/2026 that the QA round 3 checkpoint left standing — and it is
therefore the first slice of **Phase 9**, the documentation pass, rather than a fifth round of
review.

### What ran

- **The README, corrected against the code rather than reread.** Every number and every claim was
  re-derived before it was written: `pnpm run test` and `pnpm run test:int` for the counters,
  `pnpm run boundaries` for the package count, `apps/api/src/web/routes.ts` for the server-rendered
  screens (two, both printable documents), `apps/web/src/config/navigation.ts` for the per-role
  screen table, `routes/index.tsx` for where choosing a persona lands.
- **The seed stops crossing offices** — [ADR-0094](adr/0094-a-consultants-manager-is-in-the-consultants-office.md).
- **The pré-facturier's validate dialog stops being silently different** —
  [ADR-0095](adr/0095-not-computed-and-zero-are-different-answers.md).
- **`docs/` is inventoried in the README**, all of it, so a file cannot be added there in silence.

### Where I am least confident, and what it resolved to

1. **The one place the office/hierarchy distinction is still represented is a hand-built test
   fixture.** The seed no longer carries a cross-office attachment, so `org-chart.int.test.ts`'s own
   fixture is the only thing that proves ADR-0090's decision does anything. Delete it and nothing
   fails. → **new ADR** (0094), which says so in its § Consequences rather than leaving it implied.
   This is ADR-0090's second reconsideration threshold firing as written, not a surprise.
2. **Adding a person to the roster shifted every subsequent seeded UUID**, because `ids.next()` runs
   in declaration order and Thomas is declared beside Karim. Checked rather than assumed: no ADR, no
   README line and no test pins a seeded id (`git grep` for the UUIDv7 shape returns only
   `server.test.ts`'s hand-written request ids). The grades and `Cjm` values of every filler
   consultant also shift by one position in their cycle, which changes invoice amounts — no test or
   document pins those either. → **no outcome is owed**, and that is stated rather than dressed as
   one: the four outcomes resolve a _point raised_, and this one dissolved on inspection — the
   question was "does anything pin a seeded id or a filler's `Cjm`?", the answer is no, and there
   is therefore no defect, no decision and no deferral. It is recorded because the next person to
   insert a roster member will want to know it is safe, and why, not because it is unresolved.
3. **The README's « Ce que je ne construis pas » still claims 478 arbitrations, 242 of them
   rejected**, from a triage inventory that is not in this repository. A cold reader cannot check
   it. It is the owner's own claim about his own process, not a statement about the code, and it was
   left alone deliberately rather than quietly softened. → **a row in `docs/open-questions.md`**:
   the row below.
4. **`apps/web/src/assets/security-alert.webp` is untracked and unreferenced**, and appeared in the
   working tree during this session from outside this branch's work. → **a row in
   `docs/open-questions.md`**: the row of 04/09/2026 below. It was left untracked and uncommitted,
   which is a holding position and not an outcome — "not this branch's call" names who decides, not
   what was decided, and a file that is neither referenced nor ignored survives on one machine and
   follows the next `git add .`.

### In three months, what breaks if I leave it as it is

- **The README's screen table is the second thing that has to be edited when a nav entry changes**,
  and nothing enforces the pairing. `apps/web/src/config/navigation.test.ts` asserts the array; no
  test reads the README. This is the same class of rot that produced the "sept pages rendues par le
  serveur" sentence this round deleted — a prose statement of a fact the code already holds. →
  **a row in `docs/open-questions.md`**: the row below.
- **ADR-0090's closing illustration is now historical and says so nowhere in ADR-0090.** ADR-0045
  forbids editing an ADR after the fact, so the correction lives in ADR-0094; a reader who opens
  0090 alone and stops there will believe Bruno is missing a report he is not missing. Accepted as
  the cost of the no-edit rule, and named in 0094's § Decision so the ADR index is not the only
  path to it.

### Which Phase 9 tasks did not run, and why

Phase 9 has five tasks (`docs/BUILD-PLAN.md`). **9.2 bis is done** (the repository went public on
03/09/2026, ADR-0086) and **9.3 exists** as `docs/demo-checklist.md`. This round did part of **9.1**
— the corrections, not the arguments the task also owes the README (the three billing engines, the
three denominators, the ROI framing, the "why not Odoo" answer beyond the paragraph already there) —
and part of **9.4**: `docs/adr/README.md` is current and `open-questions.md`'s § Open was reviewed
row by row for the three rows this round settled, **not** row by row for the thirty-eight that
remain, and the e-invoicing calendar was **not** rechecked, as 9.4 asks. **9.2 ran on 04/09/2026**,
in the review pass below: the README now opens on the five-line reading path the task names, and
carries the CI badge 9.2 bis said would follow going public. Phase 9 is therefore **not closed** —
9.1's arguments and 9.4's row-by-row review remain — and this is not a phase checkpoint.

### The review pass, 04/09/2026 — what the two subagents found, and where each finding went

CLAUDE.md requires both reviewers before a merge to `main`. They ran on the finished branch, in
clean contexts, and between them raised twenty-four points. Neither was told what the other was
looking at. What follows is every point that changed something, and every point that did not.

**Corrected in the same pass (fix now):**

- **The README said `nightly.yml` had never run.** It has: `workflow_dispatch` on 31/08/2026, then
  `schedule` every morning since 01/09. The claim was written when it was true and left standing
  four days after it stopped being. Found by `cold-reader` running the command the section above it
  offers — which is the exact failure mode the README exists to rule out, on the only negative
  claim it makes about its own CI. The paragraph now carries the platform-measured score (75,65 %,
  from the run's own `mutation-report` artefact) and says plainly that it used to say the opposite.
- **The README counted ten gates in `ci.yml`; there are eleven.** `Shell scripts` — shellcheck over
  the root-executed deploy scripts — is a `ci.yml` job, was attributed to another workflow, and had
  no row in the table that claims to inventory the gates, while being one of the fourteen checks
  that block a merge. Table row added, the count corrected in the four places it propagated to, and
  the Playwright row renamed to the context string branch protection actually requires.
- **ADR-0094 stated a rule its own test asserts is false.** « A `manager_attachment` never crosses
  an office » — three seeded rows do (each manager → the director in Paris), and
  `tests/seed-office-scope.test.ts` asserts they _must_, because the exemption is safe only while a
  manager holds no assignment. Corrected in place under ADR-0045, and in the three places the
  overstatement had been copied to: `CLAUDE.md`, the seed's own comment, and the README.
- **ADR-0094 § Consequences claimed more than the fix delivers.** « Every seeded `Cra` is now
  validatable » is vacuous — all but one arrive `Validated`, hence immutable. Rewritten, with the
  gap it does leave open (no selectable persona can validate Rennes or Bordeaux) named in the ADR
  and opened as a row above.
- **Two assertions passed on an empty population.** In `tests/seed-office-scope.test.ts`, renaming
  the `consultant` role literal would have emptied the collection under test and left both green.
  Guarded, the way the file's second test already guarded itself.
- **One e2e assertion could not fail.** `toBeHidden()` passes on a locator matching nothing, and the
  string was hand-copied; `journeys.spec.ts` now reads the label constant the component renders.
- **Seven new comments restated ADR reasoning instead of linking to it**, and cited « QA round 4 »,
  a term that resolved nowhere: `docs/qa-rounds.md` documented three rounds. The comments now name
  ADR-0094 and ADR-0095, and `docs/qa-rounds.md` has a round 4 section that says what it actually
  was — the first slice of Phase 9, numbered as a round only because the branch already was.
- **Eight references to a `docs/demo.md` that never existed** (it shipped as
  `docs/demo-checklist.md`) in four ADRs, `docs/BUILD-PLAN.md` and `apps/api/src/personas/access.ts`.
  Corrected in place: under ADR-0045 a pointer to a file under the wrong name is a description of
  the code, not a decision.
- **`docs/BUILD-PLAN.md` — the file the README names as the calendar authority — said the hosted
  instance serves nothing and the gates are advisory.** Both were superseded by Phases 8 and 9;
  dated corrections added rather than the sentences rewritten, since the calendar is a record of
  what was planned.
- **Five readability findings from the cold read**, all cheap and all taken: the reading path at the
  top (task 9.2), Alice introduced before three paragraphs are spent on her month, the « item 6 »
...
- **The 478/242 triage claim** — already an Open row, and softening the owner's claim about his own
  process while editing his README is a rewrite, not a correction.
- **« The README argues with a previous draft of itself »** — six passages narrate corrections a
  stranger never saw. Read together they are heavy; each is the repository's stated method, which is
  to leave the correction visible rather than quietly fix it. Kept, and this paragraph is one more
  of them.
- **A commit body overstates one check** (`3f1bea7`: a surname, not the name pair, was verified free
  against the filler pool). No functional consequence, and the history is published; recorded here
  rather than rewritten.

## QA round 5 checkpoint — `fix/qa-round-5`, 04/09/2026

A fresh read of the running application, six items — `docs/qa-rounds.md` § Round 5 is the index,
`docs/todo.md` items 38–43 record each one in the house's own French.

### What ran

- **The company-news carousel's arrows, timer and transition** — item 1,
  [ADR-0096](adr/0096-the-company-news-carousel-gets-fixed-arrows-and-a-css-timer.md). Also closes
  the `security-alert.webp` row above.
- **One call to action per actionable Cra on the consultant dashboard** — item 2,
  [ADR-0097](adr/0097-one-call-to-action-per-actionable-cra.md).
- **The manager dashboard's chart becomes a staffing snapshot; billing's is a stated empty state** —
  item 3, [ADR-0098](adr/0098-the-manager-dashboard-chart-becomes-a-staffing-snapshot.md).
- **The CRA page's mobile topbar drops the period below `md`** — item 4.
- **"Choisir une mission…" becomes "Ajouter une mission…"** — item 5.
- **The CRA legend gets a card background so its colours read** — item 6.

### Where I am least confident, and what it resolved to

1. **The two new integration tests proving ADR-0098's staffing read (the name discriminator and the
   office scope) have never run.** `apps/**/*.int.test.ts` needs a live Postgres, which this
   session did not have, and the task rules kept it that way on purpose (another session may hold
   the database). The SQL was checked by hand against the migration DDL (`from_date`/`to_date`/
   `office_id`/`departure_date`/`role`/`name`, all confirmed column-for-column) and the fixture
   setup mirrors the file's own existing style, but neither is the same thing as a green run. →
   **a row in `docs/open-questions.md`**: the row below.
2. **Nothing this round changed was ever seen in a browser or through Playwright** — item 1's fixed
   arrow anchor and its `Button`-press-transform collision (found and fixed by reasoning about
   `ui/button.tsx`'s own `active:` utility, not by watching it happen), item 1c's mobile fallback,
   item 4's responsive title spans, item 6's `bg-card` legend. The reasoning is written down at each
   site; none of it is pixel-verified. → **a row in `docs/open-questions.md`**: the row below.
3. **Nine committed `tests/visual/review/*.png` files now show a screen this round changed and were
   deliberately not regenerated** (rule 7 forbids running Playwright here): `8.4-dashboard-manager.png`,
   `8.4-dashboard-consultant.png` and `8.4-dashboard-billing.png` (axe.spec.ts — items 1, 2 and 3 all
   touch the dashboard, every role), `10.4-dashboard-manager-en-attente.png` and the four
   `4.2-shell-*-{paris,lyon}.png` (item 2's queue, item 1's news panel, item 3's manager panel all
   sit in frame), and `4.5-shell-mobile-sheet.png` (the dashboard behind the Sheet, at 768px). The
   CRA-page and kitchen-sink screenshots are unaffected — checked directly: the kitchen sink renders
   none of the six items' components, and every CRA-grid capture runs at the `journeys` project's
   1440px (item 4 is `< md` only) with the legend collapsed by default (item 6 only changes the
   expanded state). → **a row in `docs/open-questions.md`**: the row below.
4. **`ManagerStaffing` lives in `apps/web/src/features/dashboard/types.ts`, not `packages/contracts`**
   — a deliberate deviation from how the task was worded, not an uncertainty. No dashboard response
   shape lives in `packages/contracts` today (only cross-cutting `ProblemDetails`/`ApiProblemType`
   do); every other per-role field on this same type is defined the same way this one now is. It was first
   recorded here as "a decision, defended, not a question", which is a fifth outcome this checkpoint
   does not have — the `rules-auditor` pass of 05/09/2026 was right to call that a silent pass. →
   **a row in `docs/open-questions.md`**: the placement row below, which carries it together with
   the two sibling placement questions the same audit raised.

### In three months, what breaks if I leave it as it is

- **The nine stale screenshots read as current** to anyone who has not read this checkpoint — a cold
  reader comparing them against the live application sees a mismatch (three overlapping calls to
  action still visible in `8.4-dashboard-consultant.png`, the invoice-history charts still visible on
  the manager/billing captures) and has no way to know they predate this round without finding this
  row first.
- **A typo in the staffing SQL is not a three-month risk** — the first real Postgres run (CI, or the
  next session with database access) fails loudly rather than silently, which is the gate this
  repository exists to prove works. Named here anyway because "unrun" and "wrong" are different
  facts and only the first one is known yet.

Both were the same underlying gap — no Playwright, no Postgres, in the session that wrote the
round. The session that pushed the branch had both, ran them, and the gap resolved in two
directions at once: the two halves it was about are answered, and the attempt to close its third
half found something the row had not suspected.

| Since      | Question                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Impact if wrong                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 04/09/2026 | **QA round 5 shipped six items and two new integration tests with no Postgres and no Playwright available in the session that wrote them.** The two `staffing-snapshot` tests (ADR-0098) had never run, and none of the six items' rendered output had been seen in a browser.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | A green `pnpm run test` and a clean `tsc`/`eslint`/`prettier` pass are not the same claim as "this runs" or "this renders as described".                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | **Settled 05/09/2026, before the merge, as the row required.** `pnpm run test:int`: **227 passed**, the two ADR-0098 tests among them — and both discriminate, verified by mutation on the real code per this repository's own precedent: widening the office scope (`c.office_id = $1 OR TRUE`) fails 4 tests including both new ones; dropping the name discriminator (`bool_or(TRUE)`) fails 6, including both. CI's `Integration tests` job re-ran them against a service container. The full Playwright suite ran in the CI's served-build topology (`E2E_SERVED_BUILD=1`): **117 passed, 32 skipped, 0 failed**, `journeys` included. Item 3's staffing panel was read on screen — "Répartition de l'équipe", En mission 7 / Intercontrat 12.                                                                                                                                                                                                                                                          |
| 05/09/2026 | **The dashboard review screenshots are not a baseline: the company-news carousel advances _during_ `page.screenshot()`, so the same code photographs differently run to run.** Found while trying to close the row above. `animations: 'disabled'` stops CSS/Web animations; it cannot stop the panel's own JavaScript auto-rotation timer (item 1, ADR-0096), which fires mid-capture and makes React remount the row keyed on the message id. Measured: **8 identical runs of the manager dashboard produced 2 distinct images**, and the rotation fired during **8 captures out of 8**. The full-suite run produced a third, worse variant — `8.4-dashboard-manager.png` with the panel's title, arrows and dots present and **no message at all**, a frame caught between the old row unmounting and the new one painting. The application itself is correct: probed in a real browser, the row carries its text at `opacity: 1` both before and after Playwright's own animation handling.                 | Every committed capture that frames this panel records a coin flip as if it were a reference. A cold reader comparing them against the live application sees a difference that is an artefact of the harness, and a genuine regression in that panel would be indistinguishable from the noise. It also means the count in the row above was wrong in the other direction: re-running produced **27** changed captures, not the nine the checkpoint predicted — the extra ones are real drift from rounds 3 and 4, which never ran `journeys` locally either (PR #18 says so in its own body).                                                                                | **Open — Clement's call, by 30/09/2026 (Phase 10, task 10.1).** Nothing was staged: committing a screenshot known to show an empty panel the application actually renders would put noise in the record, and folding rounds 3-4 drift into a round-5 commit would misattribute it. The fix is a decision with a cost, which is why it is not taken here: forcing `reducedMotion: 'reduce'` on the `desktop`/`mobile-shell` projects would stop the timer deterministically (the panel already gates auto-rotation on `useReducedMotion()`), but the same gate removes the timer bar from the render — the review captures would stop showing a feature item 1 exists to deliver. The alternative, pausing the module before each capture through the hover pause ADR-0096 already specifies, keeps the bar and costs a hover state in every dashboard screenshot. Whichever is chosen wants an ADR, and re-capturing then wants two commits: round-5 content, and rounds 3-4 drift, each saying which it is. |
| 05/09/2026 | **Three placement decisions were taken implicitly, and ADR-0065 says one of them the other way.** Raised by the `rules-auditor` pass run before this branch merged. (a) `managerStaffingSnapshot` is a one-caller dashboard read, and ADR-0065 § Decision names that case explicitly — _"`GET /api/v1/dashboard` is not extracted… written directly in `routes/api.ts`"_, moving to `composition/` only _"in the same commit that adds the second caller"_. It went to neither: it went to `apps/api/src/staffing/`, a directory ADR-0065 does not name. (b) `ManagerStaffing` is typed in `apps/web/src/features/dashboard/types.ts` rather than `packages/contracts` — point 4 of this checkpoint, which had resolved to nothing. (c) Item 4 added a **required** `mobileTitle` prop to two shared shell components, a `ResponsiveTitle`, and a pathname special case in `_shell.tsx`, with no ADR — and the tell is that the ADR is already written, as a comment at `_shell.tsx:54-64`, threshold included. | Each is defensible on its own; together they are three structural choices the record does not carry, on a repository whose stated discipline is one decision, one ADR, written at the time. (a) is the sharper one: a reader who follows ADR-0065 to decide where the _next_ dashboard read goes will be contradicted by the tree.                                                                                                                                                                                                                                                                                                                                            | **Open — Clement's call, by 30/09/2026 (Phase 10, task 10.1).** Deliberately not resolved by an agent writing three ADRs after the fact: an ADR dated after the decision it records is the failure this round already booked against itself once. (a) wants either a new ADR superseding ADR-0065's threshold or a move into `routes/api.ts`; (c) wants its comment promoted to an ADR nearly verbatim; (b) is the cheapest and may simply be affirmed. `CONTEXT.md` gained its `ManagerStaffing` entry on 05/09/2026, so the vocabulary half of (b) is already closed.                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 05/09/2026 | **`apps/web/src/assets/security-alert.webp` is a 53 KB photograph of unrecorded provenance, committed to a public repository.** The repository's own record of it, in the row settled above, is that _"it appeared in the working tree during the QA round 4 session, from outside that branch's work"_. Nothing anywhere states where it came from or under what licence; the three sibling assets it joins are 756-926 byte hand-authored SVGs. ADR-0096 § Context explicitly declined to open the question, calling the illustration _"a content swap, not a structural decision"_.                                                                                                                                                                                                                                                                                                                                                                                                                          | The repository reasons about exactly this elsewhere and reaches the opposite conclusion: `.gitignore` excludes `docs/images/` because _"the repo is public (ADR-0086), and a screenshot of a third-party product is not ours to redistribute."_ This is the one finding of the 05/09 audit with a consequence outside the repository — a redistribution question on the instance a reader is invited to open.                                                                                                                                                                                                                                                                 | **Open — Clement's call, by 30/09/2026 (Phase 10, task 10.1), and he is the only one who can answer it:** either the file's origin and licence are known and belong in the repo next to it, or the illustration is replaced by a hand-authored SVG like its three siblings. Left in place rather than removed unilaterally, since removing it would break the screen item 1 shipped; named here so the omission stops being silent.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 05/09/2026 | **Globally correct, preserved sorting on the three server-paginated tables (invoices, CRA list, both pré-facturier tables) is deferred (CEO audit F06, older audit P1.4/A8).** `DataTable`'s client-side `getSortedRowModel()` only ever had the current page to sort — clicking a column header on Henri's invoice list sent zero API requests and reordered ten rows out of a much larger result set, not the result set itself. The honest fix (a narrow, allowlisted sort specification threaded through the URL, `GET` query and repository `ORDER BY`, applied before `LIMIT/OFFSET`, with a stable id tie-breaker) is a structural change to three repository methods and their route contracts — not a copy fix, and not Clement's decision to make inside this pass.                                                                                                                                                                                                                                   | Until it is threaded through, these three tables offer **no** column sort at all (`DataTable`'s new `sortable={false}`, chosen over a false-scope caption) — a real loss of a nice-to-have, not a correctness bug, since the alternative (leaving the client-side sort live) let a control look global while quietly ordering one page. A reader who wants a chronological invoice list has to rely on the existing server-side year/period filters instead.                                                                                                                                                                                                                  | **Open — Clement's call, by 30/09/2026 (Phase 10, task 10.1).** Threading a sort spec through `InvoiceRepository.list`/`CraRepository.list`/the pré-facturier composition is a real, if bounded, piece of work across three repository interfaces and their route contracts — it owes its own ADR (the allowlist of sortable columns per table, and whether the tie-breaker is the id or a composite key) rather than being decided as a side effect of a polish pass. Invoice-detail's own lines table and the marge screen's mission table are unaffected: both hold a single record's complete, unpaginated rows, so their existing client-side sort is already correct and stays exactly as it is.                                                                                                                                                                                                                                                                                                       |
| 05/09/2026 | **ADR-0097 says `ActionCard` is untouched, and this branch changed it — twice — with no superseding ADR.** Raised by the `rules-auditor` pass run before this branch merged. ADR-0097 § Decision states in terms: _"`ActionCard`/`callToAction` is untouched: it still names whatever the currently-displayed period's own status is, on every visit"_, and its Context analysed this very button and concluded it is **not** a duplicate. Commit `874b544` made its button conditional on `validated`; the review commit went further and stopped rendering the card at all for every other status, because keeping it without its button left a bordered box whose entire content was `labels.hints[status]` — the same string the work-queue row two inches above already carries. The behaviour is now right on screen and the record is wrong: the reversal exists only as a comment in `dashboard-screen.tsx`.                                                                                            | ADR-0045 is explicit that a decision which **moved** owes a new ADR that supersedes the old one, and that only a statement which was **never true** is corrected in place. Both apply here and they resolve differently: ADR-0097's Context claim that the queue row used _"a different label (« Ouvrir mon CRA », not « Ouvrir ce CRA »)"_ was never true of the current-month row (`consultantQueue` uses `labels.open` for it; only the refused-elsewhere row uses `labels.openRefused`) — that half is a correction. The Decision sentence is the half that moved. A reader who follows ADR-0097 to decide what `ActionCard` may show will be contradicted by the screen. | **Open — Clement's call, by 30/09/2026 (Phase 10, task 10.1).** Deliberately not resolved by an agent writing the superseding ADR after the fact: that is the failure this repository has already booked against itself twice (the 05/09 placement row above says so in its own words), and ADR-0097 is a product decision about what a consultant sees, which `CLAUDE.md` reserves to Clement. What it needs is one ADR superseding ADR-0097 on that one sentence, naming the rejected option (keep the card, reword it so it is not a verbatim repeat) and the threshold. The in-place correction of the never-true label claim is bundled with it rather than done unilaterally, so the two edits to ADR-0097 land together and the file is touched once.                                                                                                                                                                                                                                                 |
| 05/09/2026 | **`docs/audit-produit-ui-ux.md` and `docs/plan-densification.md` are written in French, and BUILD-RULES says everything but `README.md` is English.** Raised by the same `rules-auditor` pass. BUILD-RULES § Working discipline: _"Everything in English except `README.md`"_; `CLAUDE.md` is stronger still — _"with **one** deliberate exception"_. Both files were created in French by `95bac9e`, already on `main`, so the bend predates this branch; this branch **extends** it by ~135 lines (the 05/09 status block and the A3 narrowing). I searched for a recorded exception covering documentation and found none: ADR-0026 and ADR-0060 make the **screens** French and say nothing about docs.                                                                                                                                                                                                                                                                                                     | The rule's own escape clause is that a rule which blocks you _"is either right, or it needs a new ADR"_ — and no ADR exists, so the repository is currently in the position it forbids: a stated rule that quietly stopped applying to two files. That is the same failure mode the CI trigger comment on `ci.yml` describes for a branch pattern nobody noticed had gone dead.                                                                                                                                                                                                                                                                                               | **Open — Clement's call, by 30/09/2026 (Phase 10, task 10.1).** Three outcomes are available and the choice is his, not an agent's: translate both files to English (they are planning notes, ~1 400 lines together — real work, no reader gain if the only reader is French-speaking); widen the exception by ADR to cover French-language working notes addressed to the same reader `README.md` addresses, naming the rejected option; or narrow the rule to what it actually protects (code, comments, commit messages, ADRs, `CONTEXT.md`) and say so. Not translated unilaterally here: doing so inside a defect-fix branch would bury a rule change in a polish pass, which is the thing this row exists to stop.                                                                                                                                                                                                                                                                                     |

## Rules-audit checkpoint — `fix/mobile-cra-and-interactions`, 07/09/2026

Two blind `rules-auditor` passes and a verification pass that ran what neither audit could.

**The arithmetic, stated once so the rest of this section can be checked against it.** The first
pass, over the whole branch (`main...HEAD`, 21 commits, 44 files, +2594/−534), raised **ten**
findings — the tenth being five small observations grouped under one heading, so **fourteen points**
at the granularity anything can be acted on. Running the two new e2e suites found a **fifteenth**.
The second pass, over the seven fix commits themselves, raised **six more**, of which two were code
and four concerned this record. **Twenty-one points in total: fifteen fixed, six carried as the six
rows dated 07/09/2026 above.** Every one resolves to exactly one of the four outcomes; none passes
silently.

### What the audit could not check, stated first

The auditor had no served build and no Docker, so **~592 lines of new Playwright/axe assertions
plus the widened `responsive.spec.ts` were unverified by it**. That gap was closed separately in
the same session: `e2e/mobile-cra-entry.spec.ts` and `e2e/mobile-usability.spec.ts` were run
against a real served application — **17 passed, 0 failed, 17 skipped by their own project guard**
— which is how finding 11 below was found. `pnpm run check` passes whole (env, lint, boundaries,
format, typecheck, 648 unit tests, coverage thresholds).

Two files were untracked and therefore outside the audit: `AGENTS.md` and
`docs/HANDOFF-qa-round-3.md`. The second already has its own row above, dated 04/09/2026.

### Fixed now

1. **A new guard shipped with no test of any kind.** `0ae277c` rebuilt by hand the `required` gate
   that `af4237f` removed with the native `<select>`, and shipped it untested — deleting it left
   every gate green while "Affecter" silently did nothing. `assignmentFormRefusal`
   (`features/affectations/form.ts`) is the decision extracted whole, with six cases; the handler
   keeps no condition of its own, so the tests gate the shipped path (`4866886`).
2. **A five-condition activation guard with one condition tested, under a commit subject claiming
   otherwise.** `7ac61a2`'s subject read "and test what the fix proves"; its only spec change was
   an unrelated selector fix. `shouldActivateRow` (`components/data-table/data-table.tsx`) is now a
   pure predicate with ten cases — verified discriminating by deleting each guard in turn, each
   deletion failing exactly one test (`d472602`).
3. **A defect documented as "tracked separately" that nothing tracked.**
   `docs/ssr-printables-sunset.md` named the invoice printable's 360px overflow and said it was
   tracked elsewhere; it was in none of the four outcomes. It now has its own row above, and the
   sunset document points at it (`7efa7f0`).
4. **A dead-symbol sweep whose own commit message claimed it was complete.** `5a46f5d` listed
   `LABELS.margin` among the symbols it removed and did not remove it; all thirteen keys of the
   server-rendered copy were dead, as was `p.actions` after both its call sites went. Removed, with
   the two comments naming `p.actions` reworded so a reader does not grep for a rule that is gone
   (`8a12d9f`). The SPA's own `LABELS.margin` is live and untouched.
5. **Two byte-identical helpers, each documented as "one place".** `invoiceDestination` existed in
   the invoice list and the pré-facturier, both carrying the docblock "one place, so the two can
   never point two different directions". One function now, in `features/factures/destination.ts`,
   with a test holding that property (`153ca9e`).
6. **Four small ones** (`4359f3b`): the row-tools panel called an absence a mission;
   `DropdownMenu modal={false}` arrived on the shared persona menu undocumented — it now names what
   the Radix default turns off; a guard had been inserted between a comment and the check it
   explains; and `CraDayCards` accepted `renderRowTools`/`compact` and silently ignored both.
7. **A test that navigated with a search parameter that does not exist** — found by running the
   suites, not by either audit. `mobile-usability.spec.ts` used `/cra?period=2026-08`, but `/cra`
   filters on `year`/`month` and degrades an unknown key silently to "no filter", so `.first()`
   opened whichever month sorted first. That was August when the test was written and October when
   it ran, which is how an assertion on `lundi 03` reached a month whose 3rd is a Saturday
   (`dd1e671`).

### What the second pass found, over the fixes themselves

Six points, and four of them were about this record rather than about code.

8. **Two extractions were left half-done** — `useRef` still re-declared `RowActivationOrigin`
   inline, and the day-card test pinned its month but not its consultant. Both fixed (`5eb6835`),
   verified on a freshly reset database after repeated local runs of two write-heavy suites had
   polluted December and produced a failure that was state, not code.
9. **Six open rows named a trigger where the rule requires a phase and a date.**
   `docs/BUILD-PLAN.md:143` is explicit that "an open row with no named phase is a deferral
   pretending to be a record", and an earlier draft of the printable-overflow row _argued_ the
   absence — which is the shape the rule names, however well argued. All six now read
   **by 30/09/2026 (Phase 10, task 10.1)**, the same phase the three 05/09/2026 rows above use,
   each keeping its own earlier reopen trigger.
10. **This section's own counts did not reconcile** — a header saying ten findings above a list of
    nine fixes and five rows, and a commit body counting commits while calling them findings. The
    arithmetic is stated at the top now, once, in a form that can be checked.

It also **cleared the fixes on evidence rather than on reading**: nine mutations of
`shouldActivateRow`, three of `assignmentFormRefusal` and two of `invoiceDestination`, each killing
exactly one test — including the off-by-one on the 500 ms dwell boundary. And it checked one claim
this record makes against the dependency itself: Radix really does omit `aria-describedby` when no
description is present. One honest caveat it recorded: `invoiceDestination`'s test holds a weaker
property than its docblock claims, because a unit test cannot see the two call sites — what stops
them drifting is that there is now one function, not the test.

### Recorded, not fixed: dead surface the second pass mapped

Roughly **75 leaf keys in `apps/api/src/web/labels.ts` with no consumer** (`LABELS.preFacturier`
alone is 41 keys with one live use) and **a dozen dead rule blocks in `apps/api/src/web/style.css`**
— `.panel`, `.grid`, `form.filter`, `dl.summary`, `ul.reasons`, `form.refusal` and others. All dead
since `2d8ddf1` removed the server-rendered screens: **pre-existing, not introduced here**, and
squarely inside the blast radius of the 06/09/2026 sunset row above. It gets no row of its own on
purpose — if that row resolves as "sunset", the whole layer goes and a separate cleanup row would
have been work booked against code that is about to be deleted.

### Open — the six rows above, all Clement's

The printable's 360px overflow · two missing ADRs (the combobox, the pointer-activation model) ·
ADR-0099/0100's template deviation, their unrecorded `Amends` field and ADR-0070 left contradicting
the code · `Activity` absent from `CONTEXT.md` · the two bodyless commits carrying those ADRs ·
`docs/todo.md` as a third French file. Each is above with its impact and what would close it, and
all six now carry Phase 10 / task 10.1 / 30/09/2026.

Four are decisions `CLAUDE.md` reserves — ADRs and `CONTEXT.md` are the part of the deliverable
Clement authors, and an agent inventing a rejected option or a reconsideration threshold would be
writing the half that makes an ADR worth having. One cannot be fixed forward at all: the branch is
pushed and the repository is public, so the two commit bodies mean a force-push.

### Merging with these six open, and why that is a decision rather than an oversight

The branch is green — every gate, both new e2e suites on a clean seed — and every mechanical
finding is closed. None of the six rows is a CI blocker. Four are decisions `CLAUDE.md` reserves to
Clement, and he took the call on 07/09/2026 to **merge with them open rather than hold the
branch**: the code they
concern is correct and tested, what is missing is the record of two decisions and a pointer on
ADR-0070, and holding green code hostage to prose that only he can write buys nothing.

The rows are the mechanism that makes that safe. They are above, dated, each with what would close
it, so the ADRs are owed rather than forgotten — which is the difference between a deferral and a
record. The one that does not survive the merge is the commit-history row: a merge fixes the
history in place, so the option of rewording those two commits closes with it, and the row stays as
the record that the rule was not met.
