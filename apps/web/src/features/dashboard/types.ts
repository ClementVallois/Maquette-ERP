 ...
 * endpoints"), so this shape is transcribed from Annexe A's per-role prose only, not verified
 * against running code. No `api.ts`/`hooks.ts` for this feature until Phase 8 first calls it.
 *
 * One response type, three optional per-role slices, rather than three response types picked by a
 * discriminant Annexe A does not name: the endpoint answers "tous rôles connectés" with different
 * fields per role, and nothing in Annexe A says how a caller tells which slice is present other
 ...
 *
 * **Interdit** (Annexe A, in bold): no `cjmCents`, no `tjmCents`, no margin anywhere in this type —
