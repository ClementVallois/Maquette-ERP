...
4. **L'écran** — <https://erp.clementvallois.fr> : choisir _Bruno Leroy (manager)_, ouvrir
   « Pré-facturier », valider un CRA, voir la facture apparaître. Le script complet, avec le test
   qui prouve chaque étape : [`docs/demo-checklist.md`](docs/demo-checklist.md).
...
| Mes informations `/mes-informations` — page d'attente                               | tous                 |
| Mes notes de frais `/mes-notes-de-frais` — page d'attente                           | tous                 |
| Mes demandes d'absence `/mes-absences` — page d'attente                             | tous                 |
...
cabinet**. Les trois dernières lignes du tableau sont des **pages d'attente assumées**, sans
domaine ni API derrière elles ; la section « Ce que je ne construis pas » dit pourquoi.
...
| Émission d'un avoir : persistance et écran                                    | La règle **est** construite et testée — `Invoice.cancelByCreditNote()` refuse tout ce qui n'est pas une facture émise, et c'est ce refus qui tient l'immutabilité — mais rien ne persiste ni n'affiche l'avoir. La chaîne démontrée s'arrête à la facture ; corriger est en aval, et la table `billing.credit_notes`, écrite par personne, est partie avec ADR-0057 (le seed le démontre depuis l'élargissement du 31/08/2026 : un avoir réel, numéroté, jamais stocké — ADR-0080)                                                                                                                                                                                                                                                                                                                                                                                                        | Première facture émise à corriger sur une instance — même seuil que le premier document _envoyé_ (ADR-0055, ADR-0056)                                                                                                                                                                                               |
| Pagination du pré-facturier au-delà d'une page                                | La liste est plafonnée à cinquante lignes (ADR-0003) : il n'y a pas de « tout afficher ». Cinquante partout sauf sur `GET /api/v1/cras`, passée à deux cents par ADR-0081 — le plafond est par route depuis cet élargissement, plus « le même partout ». Le sélecteur de mois est lui aussi lu sur une page de CRA triée par mois décroissant — il peut donc omettre un mois **ancien**, jamais un mois récent, ce qui est le comportement attendu d'un sélecteur de période. Une implantation de neuf consultants tient très largement dedans. **L'élargissement du seed du 31/08/2026 l'a fait grossir au-delà de ce cas** : Paris et Lyon dépassent maintenant les cinquante lignes, et le sélecteur de mois pour ces deux implantations n'offre plus les périodes historiques (2016 à 2024) — le seuil ci-contre est atteint, pas encore corrigé. Suivi dans `docs/open-questions.md` | Une implantation dont les CRA dépassent une page — c'est le seuil que nomme déjà ADR-0053 pour cet écran, et le correctif est une requête dédiée pour le sélecteur plus une pagination visible, pas un plafond plus haut                                                                                            |
| Tri par colonne sur les tables paginées côté serveur                          | Trois tables (liste des factures, liste des CRA, les deux tables du pré-facturier) sont une **page** d'un résultat plus large. Le tri client de `DataTable` n'a que cette page sous la main : cliquer un en-tête réordonnait dix lignes sur des centaines sans envoyer une seule requête, en ayant l'air de trier le tout. Le contrôle a été **retiré** de ces trois tables le 05/09/2026 plutôt que laissé mentir sur sa portée ; les filtres serveur (année, période, statut) restent le moyen de cibler des lignes. Les tables non paginées — les lignes d'une facture, les missions d'un écran de marge — gardent leur tri, qui est correct puisqu'elles tiennent l'intégralité de leur enregistrement                                                                                                                                                                                | Une spécification de tri sur liste blanche portée par l'URL, la route et le `ORDER BY` de trois dépôts, appliquée avant `LIMIT/OFFSET` et départagée par un identifiant stable — c'est une ADR à écrire (quelles colonnes sont triables, quel départage), suivie dans `docs/open-questions.md`, ligne du 05/09/2026 |
...
une relecture de l'application qui tourne, menée le 31/08/2026.
...
(relecture du 31/08/2026) : la maquette montre donc bien un
...
part entière. Elle est écrite, elle est tenue hors de ce dépôt, et la décision qui la cadre reste
ici : **[ADR-0075](docs/adr/0075-the-vulnerability-management-procedure-and-where-it-lives.md)**.
