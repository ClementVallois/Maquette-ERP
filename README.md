...
Ce fichier est long parce qu'il argumente. Si vous n'avez que cinq minutes, ce chemin suffit et il
se parcourt dans cet ordre :

1. **Le problème** — section « [Le problème métier](#le-problème-métier) », deux paragraphes : ce
...
serveur**, et ce sont les deux seules **pages** : la **facture imprimable** (`/facture/:id`, ADR-0055) et le
...
Six entrées, et pas une de plus : la barre latérale n'offre aucune page d'attente. Ce que la
maquette ne construit pas ne prend pas de place dans son menu — la section « Ce que je ne construis
pas » le dit à sa place.
...
`docs/` tient ce dont un lecteur a besoin, et rien d'autre : le journal de construction est
conservé hors ligne, et
[ADR-0120](docs/adr/0120-the-construction-journal-leaves-the-public-repository.md) dit pourquoi.
...
Les versions sont **strictes** (`engine-strict` est activé, donc la première commande échoue au
lieu d'avertir) : **Node ≥ 24.13.1** — la version exacte
...
1. **Une frontière de module réelle, et vérifiée par la CI** — pas une convention de nommage. Le
   ...
   la protection de branche est active sur `main`, **quatorze** checks y sont exigés
   (administrateurs compris), et c'est la plateforme qui le tient
   (**[ADR-0086](docs/adr/0086-the-gates-become-blocking-on-a-public-repository.md)** ; la section
   « Tests et portes de CI » y revient en détail).
...
| Émission d'un avoir : persistance et écran                                    | La règle **est** construite et testée — `Invoice.cancelByCreditNote()` refuse tout ce qui n'est pas une facture émise, et c'est ce refus qui tient l'immutabilité — mais rien ne persiste ni n'affiche l'avoir. La chaîne démontrée s'arrête à la facture ; corriger est en aval, et la table `billing.credit_notes`, écrite par personne, est partie avec ADR-0057 (le seed le démontre : un avoir réel, numéroté, jamais stocké — ADR-0080)                                                                                                                                                                                                                                                    | Première facture émise à corriger sur une instance — même seuil que le premier document _envoyé_ (ADR-0055, ADR-0056)                                                                                                                                                                                               |
| Pagination du pré-facturier au-delà d'une page                                | La liste est plafonnée à cinquante lignes (ADR-0003) : il n'y a pas de « tout afficher ». Cinquante partout sauf sur `GET /api/v1/cras`, passée à deux cents par ADR-0081 — le plafond est par route depuis cet élargissement, plus « le même partout ». Le sélecteur de mois, lui, **n'est plus lu sur une page** : Paris et Lyon dépassent les cinquante lignes, et un sélecteur dérivé d'une page triée par mois décroissant omettait les périodes historiques (2016 à 2024). Il a sa propre requête, sans plafond (`PgCraRepository.listPeriods`), donc il offre tout ce que l'implantation a jamais rempli. Ce qui reste plafonné, c'est la table elle-même                                 | Une implantation dont les CRA dépassent une page — c'est le seuil que nomme déjà ADR-0053 pour cet écran, et le correctif est une requête dédiée pour le sélecteur plus une pagination visible, pas un plafond plus haut                                                                                            |
| Tri par colonne sur les tables paginées côté serveur                          | Trois tables (liste des factures, liste des CRA, les deux tables du pré-facturier) sont une **page** d'un résultat plus large. Le tri client de `DataTable` n'a que cette page sous la main : cliquer un en-tête réordonnait dix lignes sur des centaines sans envoyer une seule requête, en ayant l'air de trier le tout. Elles n'offrent donc **pas** de contrôle de tri, plutôt qu'un contrôle qui mentirait sur sa portée ; ce sont les filtres serveur (année, période, statut) qui ciblent des lignes. Les tables non paginées — les lignes d'une facture, les missions d'un écran de marge — gardent leur tri, qui est correct puisqu'elles tiennent l'intégralité de leur enregistrement | Une spécification de tri sur liste blanche portée par l'URL, la route et le `ORDER BY` de trois dépôts, appliquée avant `LIMIT/OFFSET` et départagée par un identifiant stable — c'est une ADR à écrire (quelles colonnes sont triables, quel départage), suivie dans `docs/open-questions.md`, ligne du 05/09/2026 |
...
| Testcontainers, mode volumétrie                                               | Une seule base à gérer (`compose.yml` la décrit déjà, ADR-0019) et aucune charge mesurée à simuler. Le mutation testing, lui, **est** construit : Stryker tourne sur `domain/` en nightly (ADR-0027), hors du pipeline de PR pour la même raison que les deux sujets de cette ligne — un pipeline de PR trop long se contourne                                                                                                                                                                                                                                                                                                                                                                   | Testcontainers : un second conteneur à orchestrer dans le même run. Volumétrie : un besoin de charge mesuré, pas anticipé                                                                                                                                                                                           |
...
sous-traitance · dictionnaire d'indicateurs · reporting analytique · workflow d'approbation
...
- **Le manager d'un consultant est toujours dans l'implantation du consultant**
  ...
  bornent quoi que ce soit. Une seule forme de rattachement traverse encore une implantation : **un
  manager qui reporte au directeur**, à Paris. Elle est sans danger pour la raison exacte qui rend
  la règle nécessaire ailleurs — un manager ne porte aucune affectation, donc aucun CRA — et c'est
  un test qui le vérifie, pas une phrase (`tests/seed-office-scope.test.ts`).
...
**Alice Martin** est la consultante parisienne du sélecteur de persona — la première que voit un
lecteur qui ouvre l'instance, et le mois qu'il regarde en premier. Son mois est **volontairement
irrégulier** : une journée partagée entre ses deux missions
...
Une porte tient deux choses à la fois : elle **échoue** sur une violation, et son échec **empêche**
le merge. Les deux sont vraies ici, et chacune se vérifie séparément.

1. **Les règles échouent réellement.** `error`, jamais `warn` — et `tests/boundary-rule.test.ts`
   rejoue des violations délibérées pour exiger qu'elles soient refusées, ce qui teste la règle
   elle-même plutôt que le code qu'elle surveille.
2. **Leur échec verrouille le bouton de merge** — la ligne du dessous dit par quoi.

Elles tournent sur chaque push et sur chaque pull request. La _protection de branche_ GitHub est
active sur `main` : **quatorze checks y sont exigés**, `enforce_admins` compris, donc « rien ne
merge en rouge » est tenu par la plateforme plutôt que par l'auteur. Décision, option écartée et
seuil →
**[ADR-0086](docs/adr/0086-the-gates-become-blocking-on-a-public-repository.md)**.
...
Un mécanisme de plus, mais pas une douzième porte du tableau ci-dessus (qui en compte onze) :
...
Il est planifié tous les jours à **03:17 UTC** — délibérément décalé de l'heure pile, où toutes
les tâches planifiées de GitHub se déclenchent en même temps sur des runners partagés, ce qui
retarde le départ réel d'une heure ou plus. Se vérifie sans me croire :
`gh run list --workflow=nightly.yml`. Dernier score **mesuré par la plateforme**, run du
08/09/2026 : **75,65 %** (`billing` 67,75 %, `timesheet` 83,25 %) — le rapport complet est
l'artefact `mutation-report` de ce run, `gh run download <id> -n mutation-report`. Depuis la
première mesure `billing` n'a pas bougé d'un point et `timesheet` a gagné quatre : c'est exactement
ce que le score de mutation montre et que la couverture ne montre pas. C'est un douzième mécanisme,
distinct des onze portes ci-dessus : il mesure, il ne bloque pas.

### La procédure de gestion des vulnérabilités

Les dépendances sont tenues par la porte `Dependency scan` du tableau ci-dessus : `pnpm audit` et
osv-scanner, à chaque push et à chaque pull request. Elle tombe sur une vulnérabilité connue dans
une dépendance résolue, et un rouge verrouille le merge. Les mises à jour sont prises à la main,
délibérément — un bump est une décision, avec la revue de dépendance ordinaire
(**[ADR-0119](docs/adr/0119-the-uninstalled-renovate-configuration-is-removed.md)** : le seuil à
partir duquel l'automatiser vaut le coût y est écrit).
