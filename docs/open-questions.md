...
| 18/08/2026 | **Integration test files cause TS6059 in per-package typechecks.** `packages/timesheet` and `packages/billing` import from `tests/harness/` which is outside their `rootDir`. The `tsc --noEmit` of each package fails; the root typecheck and `vitest` both pass because they see the whole tree.                            | CI's `typecheck` job (which runs per-package) would fail on this if the job existed. Currently no CI job runs per-package typechecks — the root `tsconfig` check passes — so this is a latent error, not a CI failure. It becomes a real failure in **Phase 5** if per-package typechecks are added to CI. | Resolve **in Phase 5**: when `apps/api/` is created and the per-package typecheck is wired into CI, either add `tests/harness/` to a shared `tsconfig` referenced by both packages, or promote the harness to a workspace member. Building the fix now would guess the app's directory shape.                                                                                                                                                                                                        |
...
**Five commits on `feat/persistence`**, carrying five ADRs (0019, 0007, 0020, 0021, and the
BUILD-PLAN correction for 0038's cardinality change), six migration files, and 277 tests (238 unit +
39 integration, all green).
...
1. **TS6059 — integration test files outside each package's `rootDir`.** Both `timesheet` and
   ...
   fails; the root typecheck and vitest both pass. → **Open question**, resolve in **Phase 5** when
   the app directory shape is decided.
...
6. **`source_cra_ids[1]` assumes single-element arrays.** The expression index guards only the first
