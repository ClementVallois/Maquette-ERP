 ...
 * transaction. That is the whole reason a subscriber may write through that transaction and must
 * perform no side effect outside it — publishing is not a hand-off, it is a function call the
 * emitter is still inside.
 *
 ...
 * resolve — and ADR-0020 names the day a subscriber needs a side effect outside the transaction
 * as the threshold for building one.
 *
