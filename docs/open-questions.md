...
**`nightly.yml`'s schedule did not run**, for the structural reason named in point 2 above —
`workflow_dispatch` exists so it is triggerable without waiting, and it is what produced the real
score this checkpoint and ADR-0027 both report.
...
| Since      | Question                                                                                                                                                                                                                                                                                                                                                                                     | Impact if wrong                                                                                                                                                                                                                                                                                                                                    | Status                                                                                                                                                                                                                                                                                                                                                          |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 28/08/2026 | **Neither the `setup` job's real composite nor `nightly.yml`'s Stryker gate has ever run on GitHub's own infrastructure.** Both are verified locally only (the `setup` composite end to end; the mutation score via `workflow_dispatch`'s local equivalent, `pnpm exec stryker run`), corroborated against `actions/runner-images`' documented `ubuntu-latest` software list for the former. | A gate reported green that was never run is the exact defect this repository's own history records twice (ADR-0040's branch-protection claim, and the nine-vs-ten README count this same phase fixed). Neither claim above says "green" — both say "verified locally, not yet on the platform" — but the gap closes only once, on the real runner. | Resolve when this branch's pull request to `main` opens: the `setup` job's first CI run is the proof for the first half; the first `03:17 UTC` after merge (or an earlier manual `workflow_dispatch`) is the proof for the second. No later phase is named because this one already owns both proofs — the event is "this branch merges," not a numbered phase. |
| 28/08/2026 | **The vulnerability-management procedure (`docs/vulnerability-management.md`) has never been exercised**: its table is empty because `pnpm audit --audit-level=high` has never gone red since Phase 0, so the four-step decision it documents has no real precedent behind it.                                                                                                               | If a real exception is ever needed, the procedure could turn out to be missing a case its author did not anticipate — the usual gap between a designed process and a used one.                                                                                                                                                                     | Not decided here: nothing schedules a vulnerability. Reopen the day `dependencies` actually goes red, whichever phase that happens to fall in — no phase is named because none can be named honestly in advance.                                                                                                                                                |
