...
**Status: closed 21/08/2026. The Blocking column is empty.**

Every one of the thirty-six findings has one of the four outcomes recorded against it, and the
✅ Resolved column names the commit. Nothing was carried forward silently: what could not be
decided became a row in [`open-questions.md`](./open-questions.md) with the phase that decides it.

The six rows this file raised as **new ADR _(proposed)_** were decided by Clement on 21/08/2026:

- **P4-10** — the seed goes **through the repository ports**. Two measurable drifts closed with it.
- **P4-09** — **both**: ADR-0046 promotes the settled row and gives it a threshold, and
  `CONTEXT.md` gains the mechanism it was contradicting.
- **P5-10, P4-06, P5-11** — the three ADRs are **corrected in place, not superseded**. ADRs are
  decisions, not logs, and answering a sentence that was never true with an appendix makes the
  operative half harder to find. That is itself a decision, so it is **ADR-0045**, and
  `BUILD-RULES.md` now states the two-branch rule: a changed decision is superseded, a false
  description is corrected.
- **P5-07c** — **ADR-0047**. The second-implementation criterion holds; "real" never meant "in
  production", `Clock` is the precedent, and the enumeration is what goes.

**This file is deleted when Phase 6 opens**, which is what the line below has always said. It stays
through this merge because it is what the pull request is reviewed against.
...
| P4-10 | MED | `scripts/seed.ts:558-604, 676-741` | The seed drives the aggregates and then **writes the rows directly**, duplicating the mapping owned by `CraRepository` and `InvoiceRepository` — with a 27-column INSERT under the comment "column order matches pg-invoice-repository.ts". ADR-0022 § Decision says it "drives the domain aggregates rather than writing rows directly". **The drift is already measurable**: the repository writes `issuance_idempotency_key`; the seed's INSERT does not. Nothing mechanical keeps them in step — the comment _is_ the check. Add a NOT NULL column and the seed fails at demo time, not at test time. | **Decided 21/08/2026 — the seed goes through the ports.** `PgCraRepository.save`, `PgInvoiceRepository.saveDraft` and `PgEventStore.persist` replace ~150 lines of hand-written INSERT, so ADR-0022's "drives the domain aggregates rather than writing rows directly" is true as written. The two drifts close with it: the seed now writes `billing.declined_days` (it prints "2 declined" and wrote **zero** rows before, so the pré-facturier's blocking-reason column was empty from a fresh seed) and there is no second copy of the invoice mapping to fall behind `issuance_idempotency_key`. `PgEventStore` gained the injected id factory ADR-0041 requires of every writer | ✅ yes `9fdd2ee` |
