...
2. **Adding a person to the roster shifted every subsequent seeded UUID**, because `ids.next()` runs
   ...
   document pins those either. → **fix now**: nothing to fix; recorded here because the next person
   to insert a roster member will want to know it is safe, and why.
...
4. **`apps/web/src/assets/security-alert.webp` is untracked and unreferenced**, and appeared in the
   working tree during this session from outside this branch's work. → **fix now**: left untracked
   and not committed. It ships nowhere; deleting someone else's file was not this branch's call.
...
remain, and the e-invoicing calendar was **not** rechecked, as 9.4 asks. **9.2 did not run**: there
is still no five-line reading path at the top of the README. Phase 9 is therefore **not closed**,
and this is not a phase checkpoint.
