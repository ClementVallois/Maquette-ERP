...
A blind `rules-auditor` pass over the whole branch (`main...HEAD`, 21 commits, 44 files,
+2594/−534) before it merges, plus a verification pass that ran what the audit could not. Ten
findings, one further defect found by running the branch's own new e2e suites. Every point below
resolves to exactly one of the four outcomes.
...
7. **A test that navigated with a search parameter that does not exist** — found by running the
   suites, not by the audit. `mobile-usability.spec.ts` used `/cra?period=2026-08`, but `/cra`
...
### Open — the five rows added above, all Clement's

Two missing ADRs (the combobox, the pointer-activation model) · ADR-0099/0100's template deviation,
their unrecorded `Amends` field and ADR-0070 left contradicting the code · `Activity` absent from
`CONTEXT.md` · the two bodyless commits carrying those ADRs · `docs/todo.md` as a third French
file. Each is above with its impact and what would close it.

Four of the five are decisions `CLAUDE.md` reserves — ADRs and `CONTEXT.md` are the part of the
deliverable Clement authors, and an agent inventing a rejected option or a reconsideration
threshold would be writing the half that makes an ADR worth having. The fifth cannot be fixed
forward at all: the branch is pushed and the repository is public, so the two commit bodies mean a
force-push.

### Merging with four of these open, and why that is a decision rather than an oversight

The branch is green — every gate, both new e2e suites — and every mechanical finding is closed.
None of the five rows is a CI blocker. Four are decisions `CLAUDE.md` reserves to Clement, and he
took the call on 07/09/2026 to **merge with them open rather than hold the branch**: the code they
