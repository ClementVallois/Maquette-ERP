...
| 07/09/2026 | **Package 09 of the cleanup audit added the same narrow-cast helper (`craRowStatus`/`invoiceRowStatus`/`craActivityStatus`/`invoiceActivityStatus`/`craLineQuarterDays`) independently in four route files** (`dashboard.ts`, `pre-facturier.ts`, `invoices.ts`, `cra.ts`) rather than once, shared. Each narrows the same real gap — a repository interface (`CraListItem.status`, `InvoiceListItem.status`, `InvoiceYearStatusCount.status`, `CraLine.quarterDays`) declared wider (`string`/`number`) than the `CHECK` constraint or business rule the column actually carries — but the fix is copy-pasted with a comment pointing at the other copies rather than centralized, because package 09's own scope was sharing wire DTOs between `apps/api` and `apps/web`, not restructuring `apps/api/src/routes/`. | Four small, near-identical private functions with no shared home is not a correctness risk (each is verified by the typecheck it exists to satisfy) but is exactly the kind of duplication a reader hits once and then wonders whether the fifth route file should write a fifth copy or reach for one of the existing four. | Not decided here — package 09's narrow DTO-sharing scope is not license to also decide where route-file helpers should live, which is package 14's own question (`docs/clean-up-audit.consolidated.local.md`: "split API registration, then separate transport from application work... share only genuinely common input schemas and HTTP helpers"). Resolve **in package 14**, when the route files these five casts live in are already being reorganized by resource — the same phase that decides where a shared HTTP helper belongs is the one that should decide where a shared narrowing helper does too, rather than this package inventing a home for it under a different justification. The narrower question package 14 will actually answer either way: fix the underlying repository interfaces (`CraListItem.status: CraStatus` instead of `string`, etc.) so the cast is not needed at all, or keep the cast and give it one shared location. |
