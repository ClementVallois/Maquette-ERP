...
4. **`ManagerStaffing` lives in `apps/web/src/features/dashboard/types.ts`, not `packages/contracts`**
   ...
   do); every other per-role field on this same type is defined the same way this one now is. Placed
   here rather than as an open row because it is a decision, defended, not a question — recorded so
