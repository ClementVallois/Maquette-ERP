...
| Testcontainers, mode volumétrie                                               | Une seule base à gérer (`compose.yml` la décrit déjà, ADR-0019) et aucune charge mesurée à simuler. Le mutation testing a quitté cette ligne : Stryker tourne sur `domain/` en nightly (ADR-0027), hors du pipeline de PR pour la même raison que les deux qui restent ici — un pipeline de PR trop long se contourne                                                                                                                                                                                                                                                                                                                                                                        | Testcontainers : un second conteneur à orchestrer dans le même run. Volumétrie : un besoin de charge mesuré, pas anticipé                                                                                                                                                                                           |
...
ce que le score de mutation montre et que la couverture ne montre pas. Ne pas lire « onze jobs verts »
