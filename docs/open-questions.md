...
| 23/08/2026 | **ADR-0023's origin check rests on a premise a mainstream browser falsifies.** The ADR reasons “Browsers send it on every cross-site state-changing request” and refuses a missing `Origin` on that ground. Firefox with `network.http.sendOriginHeader=0` — a setting privacy-hardened profiles carry, and the one this repository's author browses with — sends the literal string `null` on a **same-origin** form post. Observed 23/08/2026 on `POST /persona` at `http://127.0.0.1:3000`, the configured origin, with `Sec-Fetch-Site: same-origin` present and correct on the same request.                                                         | Every write is refused for that reader while every screen still renders. The page they get says « Accès refusé », which reads as a broken application rather than as a security control working — and it is the demonstration chain, end to end, that they cannot complete. Neither of ADR-0023's reconsideration triggers (the first privileged persona, one real user) anticipates it.         | **Clement's to decide, not the agent's**: it reopens the one control item 4 of `CLAUDE.md`'s claims rests on, and widening a CSRF check is not a fix to make while writing up the bug that found it. The shape, if it is taken: accept when `Origin` equals the configured origin, **or** when `Origin` is absent-or-`null` **and** `Sec-Fetch-Site` is `same-origin` — with `Sec-Fetch-Site: cross-site` refusing regardless of `Origin`, so the fallback also denies rather than only widening. `Sec-Fetch-*` are forbidden header names, so page script cannot forge them; a non-browser client can set either freely but carries no cookie, which is not the CSRF threat. Three cases in `personas/access.test.ts` assert the current refusal and move with it. Decide in **Phase 8**, which is where the hosted instance first meets a reader's own browser. Until then the mitigation is a clean profile, not a code change — and the 24/08 chain is to be walked in one. The refusal's **log** field was fixed on 23/08/2026 without touching the stance: `originPresent: boolean` put `Origin: null` on the _present_ side and so named the wrong cause; it now records `absent` / `suppressed` / `mismatched`. |
