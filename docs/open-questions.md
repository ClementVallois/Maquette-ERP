...
| 19/08/2026 | **There is no route that records a day or submits a Cra.** `/api/v1` reads Cras and invoices, validates a Cra and issues an invoice. The consultant persona can therefore see its own month and change nothing about it; the seeded `submitted` Cra exists because the seed wrote it, not because anyone could.                                                                                                            | The `consultant` persona is half a persona, and BUILD-PLAN 6.3 (the Cra entry grid, "a days × lines table with live totals") has no API under it. Bounded: the chain the repository is _about_ starts at a validated Cra, and it is demonstrable end to end on the seeded instance.                                                                                                                                                                                                                                                    | Resolve in **Phase 6, task 6.3**, which is the task that needs it and the task that fixes its shape. Building the routes now would mean guessing what the grid posts — a day at a time, a month at a time, a diff — which is the guess this file exists to refuse.                                                                                                                                                                                                                                                                                                |
...
| 21/08/2026 | **Migration 007 created six tables and five of them still have no reader.** `public.grades`, `grade_tjm_defaults`, `habilitations`, `consultant_habilitations` and `mission_habilitations` are created, seeded, and read by nothing; only `consultant_grades` has one, through `consultantEconomics`. As of 21/08/2026 they are also **tested** — `tests/migration-007.int.test.ts` — so the guards are load-bearing, but a tested table with no reader is still a table with no reader. | This is the 19/08 `credit_notes` row again, six times over, in the repository that names YAGNI as its sorting criterion. It is sharper here than for `credit_notes`, because `CLAUDE.md` § Dataset shape **requires** the habilitation scenario — "one certification-based habilitation that constrains an assignment" — and a constraint that nothing reads constrains nothing. The seed stores the fact; no code enforces it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Resolve in **Phase 6**, in the task that builds the staffing or mission screen: either a reader enforces the habilitation and the tables earn their place, or they move to the README's "Ce que je ne construis pas" with the dataset-shape line amended to say the fact is recorded and not enforced. The grade grid (`grades`, `grade_tjm_defaults`) is the same decision taken twice and resolves with it. |
...
| 21/08/2026 | **The seeded month is uniform, so three concepts the domain models are never exercised by the data.** Every workable June day is `worked`, `halfDays: 2`, on `activeAssignments[0]`. Consequence: `timesheet.cra_flags` is never populated, so no flagged weekend or holiday exists in any dataset; no `absence` day exists; and although Alice holds two assignments, no day is split between them.                                                                                     | `CONTEXT.md` describes the flag at length and `CONTEXT.md` § Records of time gives the split day as the **structural reason** the mission sits on the line rather than on the day — so the dataset never exercises the one justification for the shape it is stored in. `CLAUDE.md` § Dataset shape's named checklist items are all present; this is the layer below them, and it is the layer a demo shows.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Resolve in **Phase 6**, in the task that builds the Cra screen: that is the first point at which a flagged Saturday, an absence and a split day are visible rather than notional, and the first point at which their absence would be noticed by a viewer rather than by a reviewer. If the screens do not need them, the seed says so and the concepts move to the README's "Ce que je ne construis pas".    |
| 21/08/2026 | **No seeded persona produces an empty list, so the empty state is unobserved rather than absent.** `CLAUDE.md` names empty, error and permission-denied states as deliverables and not polish. Two of the three are demonstrable today — a 403 that names its rule, a 404, a typed refusal — and the third is not: every persona the seed offers has Cras or invoices to see.                                                                                                            | An empty state that has never been rendered is an empty state that is probably wrong, and it is the one of the three that a reader meets by accident rather than by trying. The `manager-lyon` persona comes closest — it reads an empty Cra list, which is beat one of ADR-0003 — but that is an _authorization_ absence dressed as an empty state, and conflating the two is exactly what the ADR-0003 demonstration exists to separate.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Resolve in **Phase 6**, with the screens: either a persona or a period yields a genuinely empty list and the screen is built and shown for it, or the row moves to the README with the omission stated. It cannot be resolved before the screens exist, because until then there is nothing for "empty" to look like.                                                                                         |
